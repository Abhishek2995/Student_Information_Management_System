	...
	body{
		...
		<style>
			* {box-sizing: border-box;}
				...
				#facReg tr:nth-child(even){background-color: #f2f2f2;}

				#facReg tr:hover {background-color: #ddd;}
...
<!---------------------------------end header  top logout----------------------------------------->
		...
		<center>
			<%
			String oldFacultyNameText="";
			   if(request.getAttribute("lastFacultyNameText")!=null){
				   oldFacultyNameText =(String)request.getAttribute("lastFacultyNameText");
			   }
			%>
		</center>
			 <form action="SearchFacultyController">
			 	<input type="text" name="name_search_text" autofocus="autofocus" placeholder="Search.." value="<%=oldFacultyNameText%>" required="required">   
		...
		<center>
			<h2 style="color:#40e0d0; font-family:Comic Sans MS;"><u>FACULTY REGISTRATION DATABASE</u></h2>
		</center>
		
		<center>
		<%
		//fetch data from request
		ArrayList<facultyReg> facultyList =(ArrayList<facultyReg>)request.getAttribute("facultyList");
		...
		<%
			for(facultyReg faculty:facultyList){
			%>
			<tr>
				<td><%=faculty.getFname()%></td>
				<td><%=faculty.getMname()%></td>
				<td><%=faculty.getLname()%></td>
				...
				<td><a href="UpdateFacRegController?email=<%=faculty.getEmail()%>"><button class="button1">UPDATE</button></a>
				<a href="DeleteFacRegController?email=<%=faculty.getEmail()%>"><button class="button2">DELETE</button></a></td>
